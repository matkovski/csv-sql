...
export default class Expression extends Pass {
    ...
    static build(walk, Select, canBeSelect = false) {
        ...
        while (!walk.empty) {
            ...
            let partial = Expression.buildBetween(last, walk, Select);
            partial = partial || Expression.buildInPartial(last, walk, Select);
            partial = partial || Expression.buildLikePartial(last, walk, Select);
            if (partial) {
                result.push(...partial);
            } else if (token.token === '(') {
                let inside = Pass.readInsides([token, ...walk.ahead], '(', ')');
                // let looksLikeFunctionCall = result.length && ['reserved', 'identifier'].includes(result[result.length - 1].type);
    ...
    // 
    static buildInPartial(left, walk, Select) {
