...
let patterns = [
    ...
    ts => {
        if (ts.length >= 3 && ['identifier', 'reserved'].includes(ts[0].type) && ts[1].token === '(') {
            ...
            return (token => ({
                kind: token.lc in aggregate ? 'aggregate' : 'function',
...
function call(allTables, columns, rows, row, funcname, args) {
    if (!aggregate[funcname]) {
        Pass.unexpected(funcname);
    }

    return aggregate[funcname](allTables, columns, rows, row, args);
}
...
export default class Expression extends Pass {
    ...
    static buildTree(tokens, eaten, Select, canBeSelect = false) {
        ...
        let result = [];
        while (tokens[0] && tokens[0].token !==',' && (aggregate[tokens[0].lc] || tokens[0].type !== 'reserved' || ['distinct', 'in', 'and', 'or'].includes(tokens[0].lc))) {
            if (tokens[0].token === '(') {
                let c = 1;
                let inside = [];
                        ...
                        if (result.length && looksLikeFunctionCall) {
                            ...
                            if (inside.length === 1 && inside[0].token === '*') {
                                ...
                                if (inside[0].lc === 'distinct') {
                                    ...
                                    expression = Expression.buildTree(inside, [], Select, true);
                                }
