...
let patterns = [
    ...
    // null
    ts => {
        if (ts[0].type === 'reserved' && ts[0].lc === 'null') {
            return {
                kind: 'string',
                run: () => null,
                length: 1,
...
export default class Expression extends Pass {
    ...
    static build(walk, Select, canBeSelect = false) {
        ...
        while (!walk.empty) {
            ...
            let token = walk.perhaps.lc(lc => aggregates[lc] || functions[lc] || ['distinct', 'and', 'or'].includes(lc) || operators.includes(lc) || lc === 'null') || walk.perhaps.not.type('reserved');
            if (!token) {
                break;
            }
